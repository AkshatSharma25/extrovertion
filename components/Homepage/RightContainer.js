...
const RightContainer = (username) => {
  ...
  useEffect(() => {
    try {
      if (username.username !== "username") {
        axios.get(`/api/follow/getbuffered/${username.username}`).then((response) => {
          setRequests(response.data.requests);
          console.log(username,response.data.requests);
        })
      }
    }
    catch (error) {
  ...
  const handleFollowAccept = async (from) => {
    try {
      const res = await axios.patch(`/api/follow/${username.username}`, {
        from: from
      })
      toast.success("Follow request Accepted!", toastOptions)
  ...
  const handleFollowReject = async (from) => {


    try {
      const res = await axios.delete(`/api/follow/${username.username}/${from}`)
      toast.warning("Follow request denied!", toastOptions)
      console.log(res.data);
      window.location.reload()
    }
    catch (error) {
...
return (
  <div className={`h-full w-full  col-span-1 p-4 pl-0 `}>
    <div
      className={`w-[100vw] h-[100vh] z-50 left-0 top-0 backdrop-blur-md  absolute flex justify-center items-center ${CreatePost ? "" : "hidden"
      ...
      <div className="p-2  flex flex-col gap-2 overflow-auto">
        ...
        {requests.length > 0 &&
          requests.map((request) => {
            return (
              <div key={request.sender} className="flex items-center justify-center gap-4 bg-gray-100 rounded-md w-full h-12">
                <img src="/profile.svg" className="invert" alt="" />
                {request.sender}
                <div className="flex gap-2">
                  <button
                    onClick={() => handleFollowAccept(request.sender)}
                    type="button"
                  ...
                  <button
                    onClick={() => handleFollowReject(request.sender)}
                    type="button"
                    className="focus:outline-none text-white bg-purple-700 hover:bg-purple-800 focus:ring-4 focus:ring-purple-300 font-medium rounded-lg text-sm w-8 h-8 dark:bg-purple-600 flex items-center justify-center dark:hover:bg-purple-700 dark:focus:ring-purple-900"
                  >
                    <img src="/cross.svg" className="" alt="" />
